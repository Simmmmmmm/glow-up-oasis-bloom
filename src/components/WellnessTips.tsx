  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Discover tips, articles, and content to support your wellness journey</p>
      </div>

      {/* Featured Daily Tip */}
      <div className="bg-gradient-to-r from-pink-100 via-purple-100 to-mint-100 rounded-2xl p-8 mb-8 shadow-sm border border-pink-200">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h3 className="text-2xl font-bold text-purple-800">Today's Wellness Tip</h3>
            <p className="text-purple-600">Start your day with intention</p>
          </div>
        </div>
        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-6">
          <h4 className="text-xl font-semibold text-purple-800 mb-3">Morning Affirmations</h4>
            ...
            <div className="space-y-4">
              {wellnessVideos.map((video, index) => (
                <div key={index} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                  <div className="w-12 h-12 bg-gradient-to-r from-pink-100 to-purple-100 rounded-lg flex items-center justify-center text-2xl">
                    {video.thumbnail}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-gray-800 text-sm leading-tight">{video.title}</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredTips.map((tip) => {
              ...
              return (
                <div
                  key={tip.id}
                  className={`${tip.bgColor} rounded-2xl p-6 shadow-sm border ${tip.borderColor} hover:shadow-md transition-shadow`}
                >
                  <div className="flex items-center space-x-3 mb-4">
                    <div className={`w-10 h-10 bg-gradient-to-r ${tip.color} rounded-full flex items-center justify-center`}>
                      <Icon className="w-5 h-5 text-white" />
                    </div>
                    <div>
                      <span className="px-2 py-1 bg-white/60 rounded-full text-xs font-medium text-gray-700">
                        {tip.category}
                      </span>
                    </div>
                  </div>
